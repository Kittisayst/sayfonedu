...
class StudentPreviousEducationPolicy extends AppPolicy
{
    /**
     * Override method ນີ້ ເພື່ອກຳນົດຄ່າ Roles, Permissions, ແລະ Exceptions ສຳລັບ StudentPreviousEducation Policy.
     */
    protected function getRolesAndExceptionsConfig(): array
    {
        ...
        return [
            // ໃຜສາມາດເບິ່ງລາຍຊື່ປະຫວັດການສຶກສາທັງໝົດ (ຂອງນັກຮຽນຄົນໃດຄົນໜຶ່ງ)? (Filter ໃນ Controller)
            'viewAny' => [
                'roles' => $viewRoles,
                'permissions' => [],
                'exceptions' => [], // Context check likely in controller
            ],
            // ໃຜສາມາດເບິ່ງລາຍລະອຽດປະຫວັດການສຶກສາສະເພາະ record ໄດ້?
            'view' => [
            ...
            // ໃຜສາມາດສ້າງບັນທຶກປະຫວັດການສຶກສາໃໝ່ໄດ້?
            'create' => [
                'roles' => $createRoles,
            ...
            // ໃຜສາມາດແກ້ໄຂບັນທຶກປະຫວັດການສຶກສາໄດ້?
            'update' => [
            ...
            // ໃຜສາມາດລຶບບັນທຶກປະຫວັດການສຶກສາໄດ້?
            'delete' => [
    ...
    /**
     * Exception: ກວດສອບວ່າຜູ້ໃຊ້ທີ່ login ຢູ່ ເປັນນັກຮຽນເຈົ້າຂອງ Previous Education record ນີ້ ຫຼື ບໍ່.
     */
    protected function isOwnerOfRecord(User $user, Model $model): bool
    {
        if (!$model instanceof StudentPreviousEducation || !$this->isStudent($user)) { // ໃຊ້ isStudent ຈາກ AppPolicy
             return false;
        }
     ...
     /**
     * Exception: ກວດສອບວ່າຜູ້ໃຊ້ທີ່ login ຢູ່ ເປັນຜູ້ປົກຄອງຂອງນັກຮຽນໃນ Previous Education record ນີ້ ຫຼື ບໍ່.
     */
    protected function isGuardianOfStudent(User $user, Model $model): bool
    {
        if (!$model instanceof StudentPreviousEducation || !$this->isParent($user)) { // ໃຊ້ isParent ຈາກ AppPolicy
             return false;
        }
        // ສົມມຸດ StudentPreviousEducation model ມີ relationship 'student'
    ...
    /**
     * Exception: ກວດສອບວ່າຜູ້ໃຊ້ທີ່ login ຢູ່ ເປັນຄູທີ່ກ່ຽວຂ້ອງກັບນັກຮຽນຂອງ Previous Education record ນີ້ ຫຼື ບໍ່.
     */
    protected function isTeacherOfStudent(User $user, Model $model): bool
    {
        if (!$model instanceof StudentPreviousEducation || !$this->isTeacher($user)) { // ໃຊ້ isTeacher ຈາກ AppPolicy
            return false;
        }
