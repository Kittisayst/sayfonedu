...
class StudentPreviousLocationPolicy extends AppPolicy
{
    /**
     * Override method ນີ້ ເພື່ອກຳນົດຄ່າ Roles, Permissions, ແລະ Exceptions ສຳລັບ StudentPreviousLocation Policy.
     */
    protected function getRolesAndExceptionsConfig(): array
    {
        ...
        return [
            // ໃຜສາມາດເບິ່ງລາຍຊື່ທີ່ຢູ່ເກົ່າທັງໝົດ (ຂອງນັກຮຽນຄົນໃດຄົນໜຶ່ງ)? (Filter ໃນ Controller)
            'viewAny' => [
                'roles' => $viewRoles,
                'permissions' => [],
                'exceptions' => [], // Context check likely in controller
            ],
            // ໃຜສາມາດເບິ່ງລາຍລະອຽດທີ່ຢູ່ເກົ່າສະເພາະ record ໄດ້?
            'view' => [
            ...
            // ໃຜສາມາດສ້າງບັນທຶກທີ່ຢູ່ເກົ່າໃໝ່ໄດ້?
            'create' => [
                'roles' => $createRoles,
            ...
            // ໃຜສາມາດແກ້ໄຂບັນທຶກທີ່ຢູ່ເກົ່າໄດ້?
            'update' => [
            ...
            // ໃຜສາມາດລຶບບັນທຶກທີ່ຢູ່ເກົ່າໄດ້?
            'delete' => [
    ...
    /**
     * Exception: ກວດສອບວ່າຜູ້ໃຊ້ທີ່ login ຢູ່ ເປັນນັກຮຽນເຈົ້າຂອງ Previous Location record ນີ້ ຫຼື ບໍ່.
     */
    protected function isOwnerOfRecord(User $user, Model $model): bool
    {
        if (!$model instanceof StudentPreviousLocation || !$this->isStudent($user)) { // ໃຊ້ isStudent ຈາກ AppPolicy
            return false;
        }
    ...
    /**
     * Exception: ກວດສອບວ່າຜູ້ໃຊ້ທີ່ login ຢູ່ ເປັນຜູ້ປົກຄອງຂອງນັກຮຽນໃນ Previous Location record ນີ້ ຫຼື ບໍ່.
     */
    protected function isGuardianOfStudent(User $user, Model $model): bool
    {
        if (!$model instanceof StudentPreviousLocation || !$this->isParent($user)) { // ໃຊ້ isParent ຈາກ AppPolicy
            return false;
        }
        // ສົມມຸດ StudentPreviousLocation model ມີ relationship 'student'
